...
static void ESP32_TTS(char *message, int voice)  // WAG 090425 replace 2nd parameter voice
{
    char filename[MAX_FILENAME_LEN];

    if (settings->voice != VOICE_ON || settings->adapter != ADAPTER_TTGO_T5S) 
      return;
...
/*
      ...
      EPD_Message("VOICE", "ALERT");  // WAGA 030425 -- here is the text during voice.  Disabled so as to not blank srceen
...
*/
      ...
      while (word != NULL)
      {
          strcpy(filename, WAV_FILE_PREFIX);  // WAGA 090425  lines below were settings->voice
...
/*  WAGA 180425 -- don't play the 'post' as its iritating
...
*/    
      ...
      strcat(filename, "notice");  // sounds like 'noted'
      strcat(filename, WAV_FILE_SUFFIX);
