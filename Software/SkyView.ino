...
void setup()
{
  ...
  char buf[8]; 
  strcpy(buf,"POST");  // WAGA  silenced in TrafficHelper.cpp
  SoC->TTS(buf,VOICE_1);

  if( settings->voice==VOICE_BUZZER){
    digitalWrite(SOC_GPIO_PIN_BUZZ_T5S, HIGH);  // WAGA 180425 added test at startup
    delay(200);
...
void loop()
{
  ...
  Serial.println("TIMER A - start Skyview.ino loop");  // WAGA debug timing 140425
  Input_loop();
