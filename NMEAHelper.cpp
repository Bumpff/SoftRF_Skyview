...
TinyGPSCustom T_NoTrack         (nmea, "PFLAA", 12);
TinyGPSCustom T_Source          (nmea, "PFLAA", 13);
...
char info_line [20];  // WAGA 140425  timing analysis
uint32_t id=0; // WAGA 140425 timing analysis
...
static void NMEA_Parse_Character(char c)
{
    ...
    Serial.println("TIMER B3 - valid Sentence found, set fo");  // WAGA debug timngs 220325
      ...
      if (T_ID.isUpdated()) {
        ...
        id = fo.ID; // WAGA timing analysis
          int ID_Alarm = fo.alarm_level;
          snprintf(info_line, sizeof(info_line), "LAA ID= %06X A= %d ", id,ID_Alarm); 
...
// --- WAGA 280325 added NoTrack and Source, available in Flarm protocol 9 or higher
        if (T_NoTrack.isUpdated())
        {
//          Serial.print(F(" NoTrack=")); Serial.print(T_NoTrack.value());
          fo.NoTrack = atoi(T_NoTrack.value());
//          Serial.print(F(" NoTrack=")); Serial.println(fo.NoTrack);
        }
        if (T_Source.isUpdated())
        {
//          Serial.print(F(" Source=")); Serial.print(T_Source.value());
          fo.Source = atoi(T_Source.value());
//         Serial.print(F(" Source=")); Serial.println(fo.Source);
        }
// ----------- end of WAGA additions
...
//          Serial.print(F(" ID=")); Serial.print(S_ID.value());
          ...
          id = fo.ID; 
          int ID_Alarm = fo.alarm_level;
          snprintf(info_line, sizeof(info_line), "LAU ID= %06X A=%d", id, ID_Alarm);
          Serial.println(info_line); 
...
void NMEA_loop()
{
  ...
  {
  case CON_SERIAL:
    Serial.println("TIMER B1 - serial read start");  // WAGA debug timing 140425
    ...
    Serial.println("TIMER B2 - serial read stop");  // WAGA debug timing 140425
...
#endif
    {
      while (Serial.available() > 0) {
        char c = Serial.read();// ---WAGA this should be reading the microUSB port but does not work.  Fixed in Linar Rev 0.13 but  not applied.
        NMEA_bridge_buffer(c);
        NMEA_Parse_Character(c);
